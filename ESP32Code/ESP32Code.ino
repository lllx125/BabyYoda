...
String processHead(const RobotControllerState &oldState, const RobotControllerState &newState)
{
    ...
    if (oldState.headLR != newState.headLR)
    {
        ...
        if (newState.headLR == 0)
            code += "l"; // “stop turning head”

        else if (newState.headLR == 1)
            code += "R"; // turn head right

        else if (newState.headLR == -1)
            code += "L"; // turn head left
    }
    ...
    if (oldState.headUD != newState.headUD)
    {
        ...
        if (newState.headUD == 0)
            code += "f"; // “stop tilt”

        else if (newState.headUD == 1)
            code += "B"; // tilt up
...
String processLArm(const RobotControllerState &oldState, const RobotControllerState &newState)
{
    ...
    if (oldState.lArm.x != newState.lArm.x)
    {
        ...
        if (newState.lArm.x == 0)
            code += "s";

        else if (newState.lArm.x == 1)
            code += "S";
    ...
    if (oldState.lArm.y != newState.lArm.y)
    {
        ...
        if (newState.lArm.y == 0)
            code += "q";

        else if (newState.lArm.y == 1)
            code += "Z";
...
String processRArm(const RobotControllerState &oldState, const RobotControllerState &newState)
{
    ...
    if (oldState.rArm.x != newState.rArm.x)
    {
        ...
        if (newState.rArm.x == 0)
            code += "j";

        else if (newState.rArm.x == 1)
            code += "K";
    ...
    if (oldState.rArm.y != newState.rArm.y)
    {
        ...
        if (newState.rArm.y == 0)
            code += "p";

        else if (newState.rArm.y == 1)
            code += "M";
...
/**
 * @brief Builds a command code string based on the changes in the robot controller state.
 *
 * This function generates a string that represents the commands to be sent to the robot
 * based on the differences between the old state and the new state of the robot controller.
 * It processes various components of the robot, such as the head, special functions,
 * left arm, and right arm, and combines their respective command strings.
 *
 * @param oldState The previous state of the robot controller.
 * @param newState The current state of the robot controller.
 * @return A string containing the combined command code for the robot.
 */
